...
io.on('connection', (socket) => {
    ...
    switch (normalizedQuizType) {
      ...
      case 'history':
        ...
        if (categories && categories.length > 0) {
          ...
          const allHistoryCategories = getHistoryCategories();
  ...
  socket.on('start_round', async ({ gameId }) => {
    ...
    // ✅ HISTORIA-QUIZ: Hämta bild om den finns
    else if (game.quizType === 'history') {
      if (selectedItem.imageUrl) {
        imageUrl = selectedItem.imageUrl;
        console.log(`📜 Historisk händelse: ${selectedItem.title} - Bild: ${imageUrl.substring(0, 50)}...`);
      } else {
        console.log(`📜 Historisk händelse: ${selectedItem.title} (ingen bild)`);
      }
    }
    
    // ✅ TODO: UPPFINNINGAR/SPEL
    ...
    if (previewUrl || spotifyUri || youtubeId || imageUrl || game.quizType === 'history') {
      io.to(game.hostId).emit('master_preview', {
        ...
        youtubeId,
        // Bild (för historia/uppfinningar)
        imageUrl,
        // Metadata
        title: selectedItem.title,
...
server.listen(PORT, () => {
  ...
  console.log(`📦 Quiz types supported: music, movies, history`);
  console.log(`🔜 Coming soon: inventions, games`);
});
